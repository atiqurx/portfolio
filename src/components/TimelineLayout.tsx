    ...
    <div className="relative mt-6 md:mt-12">
      ...
      {items.map((item, index) => {
        ...
        return (
          <div
            key={index}
            className={`relative flex flex-col md:flex-row md:items-center gap-6 mb-10 md:mb-24
              ${!isEven ? "md:flex-row-reverse" : ""}
              `}
          >
            {/* Dot */}
            <div
              className={`hidden md:block absolute left-[7px] top-2 md:top-6 w-3 h-3 bg-primary rounded-full z-10 shadow-md
          md:left-1/2 md:-ml-[7px]
        `}
            />

            {/* Text Card */}
            <div className="md:w-1/2 md:px-8 pt-8 md:pt-0">
